...
const Resultado = () => {
    ...
    $(container).css({"background-color":"#333","margin-top":"20px"});    
    const foto = $ ('<img src="assets/img/peru/Ana Paula Verme.jpg">');
    $(foto).css({"width":"300px","height":"300px"});
    const divResultado = $ ('<div></div>');
    $(divResultado).css({"float":"right","width":"50%","margin-top":"20px"});
    const lblNombre = $ ('<label class="rokkitt white">Ingresa su nombre: </label><br>');
